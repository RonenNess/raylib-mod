...
void DrawRectangleWithoutChangingTexture(Rectangle rec, Rectangle source, Vector2 sourceSize, Vector2 origin, float rotation, Color color, Vector3* normal)
{
    ...
    rlTexCoord2f(source.x / sourceSize.x, (source.y + source.height) / sourceSize.y);
    rlVertex2f(bottomLeft.x, bottomLeft.y);

    rlTexCoord2f((source.x + source.width) / sourceSize.x, (source.y + source.height) / sourceSize.y);
    rlVertex2f(bottomRight.x, bottomRight.y);

    rlTexCoord2f((source.x + source.width) / sourceSize.x, source.y / sourceSize.y);
    rlVertex2f(topRight.x, topRight.y);
